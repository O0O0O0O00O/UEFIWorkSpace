...
EFI_STATUS SendTCP4Socket(UINTN index, unsigned char* Data, UINTN Length)
{
    EFI_STATUS Status = EFI_NOT_FOUND;
    MYTCP4SOCKET *CurSocket = TCP4SocketFd[index];
    UINTN waitIndex = 0;
    ...
    // printf("Data: %s\n", Data);
    // printf("Length: %d\n", Length);
    CurSocket->m_TransData->Push = FALSE;
    CurSocket->m_TransData->Urgent = FALSE;
    ...
    CurSocket->m_TransData->FragmentTable[0].FragmentBuffer =(void*)Data;//必须强转，否则会出问题
