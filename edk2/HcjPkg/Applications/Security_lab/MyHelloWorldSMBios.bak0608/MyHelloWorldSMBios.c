...
cJSON* get_info(char* username)
{
    ...
    //向对象中增加用户名
    cJSON_AddItemToObject(json, "name", cJSON_CreateString(username));
    //添加非热插拔数组
    cJSON *array = NULL;
    ...
    EFI_FILE_PROTOCOL *FileHandle)
{
    ...
    for (HandleIndex = 0; HandleIndex < HandleCount; HandleIndex++)
    {
        ...
        //每次遇到usb设备时添加键值对
        cJSON_AddItemToArray(USBsubArray, pointTempObj = cJSON_CreateObject());
        cJSON_AddNumberToObject(pointTempObj, "VendorID", DeviceDescriptor.IdVendor);
        cJSON_AddNumberToObject(pointTempObj, "DeviceId", DeviceDescriptor.IdProduct);
        Textbuf = (CHAR16 *)L"FOUND ONE USB DEVICE!\r\n";
        BufSize = StrLen(Textbuf) * 2;
        Status = FileHandle->Write(FileHandle, &BufSize, Textbuf);
    }
